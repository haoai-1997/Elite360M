...
class Matterport3D(data.Dataset):
    ...
    def _load_semseg(self, index):
        ...
        _semseg = Image.open(semantic_name[:-4] + "_gt.png")
        _semseg = np.array(_semseg).astype('int32')[:, :, 0] - 1
        _semseg[_semseg >=41] = -1
        return _semseg

    def _load_depth(self, index):
        depth_name = os.path.join(self.root_dir, self.multiple_list[index][1])
        _depth = cv2.imread(depth_name, -1)
        _depth = _depth.astype(np.float32) / 4000
        _depth[_depth > self.max_depth_meters + 1] = self.max_depth_meters + 1
        return _depth
